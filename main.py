...
class Game():
    def __init__(self):
        ...
        # cor de fundo
        self.background_color = self.colors["violeta"]
    ...
    def draw_config(self):
        ...
        self.lista_botoes.append(btn_controles)
        self.lista_botoes.append(btn_som)
        self.lista_botoes.append(btn_creditos)
        self.lista_botoes.append(btn_voltar)

        for _ in self.lista_botoes:
            index = self.lista_botoes.index(_)
            altura = 0
            margin = 50
            if index > 0:
                altura = self.lista_botoes[index - 1].altura
    ...
    def draw_controles(self, first_draw = False):
            ...
            for evento in pygame.event.get():
                ...
                if evento.type == pygame.MOUSEBUTTONDOWN:
                    for btn in self.lista_botoes:
                        ...
                        self.n_tela, self.desenhado = btn.click()
                        if btn.action != "change_key": continue
                        
                        if btn.input_on is True:
                            btn.input_on = False
                            btn.background_color = self.colors["cinza_claro"]
                        else: 
                            btn.input_on = True
                            btn.background_color = self.colors["kirby"]

                        btn.draw(btn.surface, (btn.x,btn.y), btn.text_shadow, btn.background_color, btn.centralizado, btn.box_shadow, btn.borda, btn.padding, btn.max_width)
                            

                if evento.type == pygame.KEYDOWN:
                    if self.n_tela != 2: continue
                    if evento.key != 27 and evento.key != 112: continue
                    #testanto pseudo menu
                    s = pygame.Surface((50,50))
                    s.set_colorkey((255,0,0))
                    s.set_alpha(50)
                    self.tela.blit(s, (self.width / 2 - 25, self.height / 2 - 25))
...
# temos que ter uma lsita de botoes
# se um botao tiver clickado devemos colocar o background em azul e, se nao, colocar em cor normal
# IDEIA::
# colocar a lista de botoes com x e y, para cada botao desenhar com o self.color_background
# se for clickado e estiver desativdao --> self.background_color = blah
# no loop : for _ in btn: btn.draw(btn.sellllf)
# se for clickado e estiver ativad --> self.background_color = desblah
# se for clickado e outro estiver ativado --> o ativado desativa e o que foi clickado --> self.background_color = blah
# implementar essa lógica para a alteração e liberação do input : o btn ter um atributo : que diz se está podendo

# -----------------------------------
# se clickar num botao ativo vamos guardar a key dele e, a partir disso, pegar o input do teclado
# quando cickar em salvvar temos que alterar a chave, se tiver outra chave que tenha essa key, ela deve ficar nula
# o menu nao pode ser salvo se tiver uma chave nular
...
# desativar todas as teclas e deixar apenas o mouse
# se clickar em continuar volta para o jogo